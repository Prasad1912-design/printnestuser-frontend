...
const PostersCatlog = () => {
  const renderStars = (rating) => (
    <span className="rating-poster text-yellow-400">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
          {products.map((p) => (
            <div key={p.id} className="product-card shadow-xl">
              <div className="product-id-poster">#{p.id}</div>
              <img src={p.img} alt={p.name} className="product-image" />
              <div className="product-content-poster">
                <h3 className="product-title-poster">{p.name}</h3>
                <p className="product-desc-poster">{p.desc}</p>
                <p className="product-price-poster">{p.price}</p>
                <div className="product-rating-poster">{renderStars(p.rating)}</div>
